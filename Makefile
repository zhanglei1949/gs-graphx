...
client: learning
	cd $(CLIENT_DIR) && \
	python3 -m pip install ${PIP_ARGS} "torch" "networkx<=3.0" --index-url https://download.pytorch.org/whl/cpu  && \
	python3 -m pip install ${PIP_ARGS} -r requirements.txt -r requirements-dev.txt  && \
	export PATH=$(PATH):$(HOME)/.local/bin && \
	python3 setup.py build_ext --inplace  && \
	if [ $(WITH_GLTORCH) = ON ]; then \
		python3 setup.py build_gltorch_ext --inplace ; \
	fi && \
	python3 -m pip install  --no-build-isolation --editable $(CLIENT_DIR) && \
...
coordinator: client
	...
	python3 -m pip install --editable $(COORDINATOR_DIR) && \
